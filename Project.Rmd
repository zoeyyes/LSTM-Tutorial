...
```
If we write the formula as matrix expression (W value is weights and b value is bias):(?bias dimension)
$$
\begin{align}
...
```
If we write the formula as matrix expression (W value is weights and b value is bias):(?tilda)
$$
\begin{align}
\boldsymbol{I}_{t}&=\sigma(\boldsymbol{X}_{t}\boldsymbol{W}_{xi}+\boldsymbol{H}_{t-1}\boldsymbol{W}_{hi}+\boldsymbol{b}_{i})\\
\boldsymbol{Ctilde}_{t}&=\tanh(\boldsymbol{X}_{t}\boldsymbol{W}_{xc}+\boldsymbol{H}_{t-1}\boldsymbol{W}_{hc}+\boldsymbol{b}_{c})\\
Dimensions:\\
\boldsymbol{X}_{t(n\times d)}\\
...
\boldsymbol{C}_{t-1(n\times h)}; \boldsymbol{C}_{t(n\times h)}\\
